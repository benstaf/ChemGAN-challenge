    ...
    def get_reward(self, sess, input_x, rollout_num, cnn, reward_fn = None, D_weight = 1):
        ...
        for i in range(rollout_num):
            for given_num in range(1, self.sequence_length):
                ...
                if reward_fn:
                    ypred =  np.array([item[1] for item in ypred_for_auc]) + 1- D_weight # D_weight ==0 -> ypred>1 -> min(ypred,x)==x for any x<1, -> Discriminator is made irrelevant. D_weight ==1 -> just min, nothing special
                    ypred = np.minimum( ypred, reward_fn(generated_seqs))
                else:
            ...
            ypred_for_auc = sess.run(cnn.ypred_for_auc, feed)
            if reward_fn:
                ypred =  np.array([item[1] for item in ypred_for_auc]) + 1- D_weight
                ypred = np.minimum(ypred, reward_fn(input_x))
            else:
                ypred = np.array([item[1] for item in ypred_for_auc])
