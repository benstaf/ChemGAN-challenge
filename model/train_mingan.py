...
from rollout_mingan import ROLLOUT
from target_lstm import TARGET_LSTM
# import io_utils
import pandas as pd
import mol_metrics_new2 as mm
...
D=1  #number of iterations of the discriminator
  ...
  #      random.shuffle(negative_samples)
        ...
        for batch in dis_batches:
            ...
            _, step, loss, accuracy, ypred_for_auc = sess.run(
                [dis_train_op, dis_global_step, cnn.loss, cnn.accuracy,cnn.ypred_for_auc ], feed)
...
#    ckpt_file = os.path.join(ckpt_dir, ckpt_dir + '_model')   #old checkpoint
    ckpt_file = os.path.join(ckpt_dir, 'drd2_new' + '_model_')   #new checkpoint iterate over checkpoints to find largest total a

    nbatches_max= 0
    for i in range(500): #maximal number of batches iterations is 500
        if os.path.isfile(ckpt_file  + str(i) + '.meta'):   #and params["LOAD_PREV_SESS"]
            nbatches_max= i
...
#        saver.restore(sess, ckpt_file)
        print('Previous session loaded from previous checkpoint {}'.format(ckpt_file))
        prev_sess= True
    else:
        if params["LOAD_PREV_SESS"]:
            print('\t* No previous session data found as {:s}.'.format(ckpt_file))
        else:
...
#    nbatches_max= 30
    ...
    if nbatches_max+1 > TOTAL_BATCH:
        print(' We already trained that many batches: Check the Checkpoints folder or take a larger TOTAL_BATCH')
    else:
    ...
    #for nbatch in tqdm(range(TOTAL_BATCH)):
        ...
        if nbatch % params["EPOCH_SAVES"] == 0:
            save_results(sess, PREFIX, PREFIX +  '_model_' + str(nbatch) , results_rows)
