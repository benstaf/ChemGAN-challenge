...
class ROLLOUT(object):
    ...
    def get_reward(self, sess, input_x, rollout_num, cnn, ord_dict, reward_fn = None, D_weight = 1):
...
###  Code adapted for graph conv for discriminator:

		def calc_ypred_for_auc(generated_seqs):
		  generated_smiles= [mm.decode(sample, ord_dict) for sample in generated_seqs]

#pick the indices of the generated molecules that are so wrong that they do not even load with rdkit:
		  failed_indices=[]
		  for k in range(len(generated_smiles)):
			mol = Chem.MolFromSmiles(generated_smiles[k])
			if mol==None:
				failed_indices.append(k)
#		  print('failed indices in generated: ' + str(failed_indices))
	
#

		  df_generated= pd.DataFrame({'smiles': generated_smiles, 'nothing': [0]*len(generated_smiles) })

# 		  pickle.dump( df_generated, open( "rollout_df.p", "wb" ) )

		  loader = deepchem.data.PandasLoader( tasks=['nothing'], smiles_field='smiles', featurizer=deepchem.feat.ConvMolFeaturizer())
		  generated_dataset = loader.featurize(df_generated, shard_size=8192)
		  feed_dict_generator=cnn.default_generator(generated_dataset, epochs=1)

		  out_tensors = [x.out_tensor for x in cnn.outputs]
		  results = []
		  for feed_dict in feed_dict_generator:
#		    print(str(feed_dict))
	            feed_dict = {cnn.layers[k.name].out_tensor: v for k, v in six.iteritems(feed_dict)}

#		    print('notre feed_dict est ' + str(feed_dict))

	            feed_dict[cnn._training_placeholder] = 0.0
                    ypred_for_auc = sess.run(out_tensors, feed_dict=feed_dict)
	            result = np.array(sess.run(out_tensors, feed_dict=feed_dict))
	            if len(result.shape) == 3:
	              result = np.transpose(result, axes=[1, 0, 2])
	            results.append(result)

#		  print('forward pass for cnn ended')

		  certain_failure=np.array([[1.0,0.0]])  #in this case, we know from rdkit that the generated molecule is not viable 
		  if len(results)>0:
		    #	failed_indices=[0,1,2]  #pour tester
			y_final=np.concatenate(results, axis=0)
			for index in failed_indices:
				y_final= np.insert(y_final, index, certain_failure, axis=0)
		  else:
			y_final= [certain_failure]*(len(failed_indices))
			y_final=np.concatenate(y_final, axis=0)
		
	          ypred_for_auc = y_final  #np.concatenate(results, axis=0)
		  return ypred_for_auc

		ypred_for_auc= calc_ypred_for_auc(generated_seqs)
#Generated seqs pre-processing:
#		generated_smiles= [mm.decode(sample, ord_dict) for sample in generated_seqs]

#Pickle generated sequences
#		f = open('gen_seqs_rollout.p', 'w')   # Pickle file is newly created where foo1.py is
#		pickle.dump(generated_seqs, f)          # dump data to f
#		f.close()    
...
#		f = open('ypred_auc_rollout.p', 'w')   # Pickle file is newly created where foo1.py is
#		pickle.dump(ypred_for_auc, f)          # dump data to f
#		f.close()    

#    ypred_for_auc est de la forme: array([[  9.99987960e-01,   1.20096593e-05],
#                                    [  9.99999881e-01,   6.53804051e-08]], dtype=float32)
...
#            ypred_for_auc = sess.run(cnn.ypred_for_auc, feed)
	    ypred_for_auc= calc_ypred_for_auc(input_x)
