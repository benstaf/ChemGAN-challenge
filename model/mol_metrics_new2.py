...
def verify_smiles(smiles):
    ...
    for smile in smiles:
        if verify_sequence(smile):
            verify_smiles.append(smile)
    return verify_smiles
...
def compute_results(model_samples, train_data, ord_dict, results={}, verbose=True):
    ...
    verified_samples = [sample for sample in samples if verify_sequence(sample)]
    unverified_samples = [
    ...
    metrics = ['novelty', 'hard_novelty', 'soft_novelty',
               ...
               'naturalness', 'synthesizability', 'qed','DRD2']
...
def filter_drd2_80p_file(filename):
    smiles= read_smi(filename)
    smiles=verify_smiles(smiles)
    return filter_drd2_80p(smiles)


def filter_drd2_80p(smiles):
    smiles_80p=[]
    values_drd2=batch_DRD2(smiles)
    for i in range(len(smiles)):
        if values_drd2[i]>0.8:
            smiles_80p.append(smiles[i])
    return smiles_80p


def filter_drd2_80p_values(smiles):
    smiles_80p=[]
    values_drd2=batch_DRD2(smiles)
    for i in range(len(smiles)):
        if values_drd2[i]>0.8:
            smiles_80p.append(smiles[i])
    return smiles_80p, values_drd2


def filter_drd2_80p_values_file(filename):
    smiles= read_smi(filename)
    smiles=verify_smiles(smiles)
    return filter_drd2_80p_values(smiles)


#======self-diversity


#====== Self-diversity metric



def self_diversity_file(filename):
    smiles= read_smi(filename)
    smiles=verify_smiles(smiles)
    return self_diversity(smiles)


def self_diversity(smiles):
    if len(smiles)>3000:
        rand_smiles = random.sample(smiles, 3000)
    else:
        rand_smiles = smiles
    rand_mols = [Chem.MolFromSmiles(s) for s in rand_smiles]
 #   print(rand_mols)
    fps = [Chem.GetMorganFingerprintAsBitVect(
        m, 4, nBits=2048) for m in rand_mols if m != None]
    vals = [diversity(s, fps) for s in smiles]
    return np.mean(vals)
...
#===QED===  http://silicos-it.be.s3-website-eu-west-1.amazonaws.com/software/biscu-it/qed/1.0.1/qed.html


def batch_qed(smiles, train_smiles=None):
    mol_drug_qed = [qed.default(Chem.MolFromSmiles(sample)) if verify_sequence(sample)  else 0 for sample in smiles]
    return mol_drug_qed


#====== DRD2 reward, see: https://github.com/MarcusOlivecrona/REINVENT/blob/master/scoring_functions.py


def batch_DRD2(smiles, train_smiles=None):
    vals=[0]*(len(smiles))
    valid_vals, valid_idxs = drd2.activity_scores(smiles) 
    for i in range(len(valid_vals)):
        vals[valid_idxs[i]]=valid_vals[i]
    return vals



def batch_qed_DRD2(smiles, train_smiles=None):
    result= []
    bq= batch_qed(smiles, train_smiles=None) 
    bd= batch_DRD2(smiles, train_smiles=None)
    for i in range(len(bq)):
        result.append(bq[i]+bd[i])
    return result
...
def load_reward(objective):
    ...
    metrics = {}
    metrics['kindness'] = freq_compare_verified_list
    ...
    metrics['qed']= batch_qed
    metrics['DRD2']=batch_DRD2

#   metrics['qed+DRD2']=  batch_qed_DRD2
...
#language frequency index num


def freq_vector_train(train_file):
    train_list=load_train_data(train_file)
    bigstring= ''.join(train_list)
    occur_dict = collections.Counter(bigstring)
    occur_vect=[occur_dict[x] for x in occur_dict.keys()]
    freq_list=[float(x)/len(bigstring) for x in occur_vect] 
    freq_vector=np.array(freq_list)	
    return freq_vector, occur_dict.keys()


def freq_vector_gen(gen_file,train_dict_keys): #We need the keys of the training data.
    gen_list=load_train_data(gen_file)
    bigstring= ''.join(gen_list)
    occur_dict = collections.Counter(bigstring)
    occur_vect=[occur_dict[x] for x in train_dict_keys]  #We need the keys of the training data. Some letters might never be generated. In this case, we get 0.
    freq_list=[float(x)/len(bigstring) for x in occur_vect] 
    freq_vector=np.array(freq_list)	
    return freq_vector


def freq_compare(gen_file,train_file):
    freq_vector_train_list, train_dict_keys= freq_vector_train(train_file)
    freq_vector_gen_list=freq_vector_gen(gen_file,train_dict_keys)
    diff= freq_vector_train_list- freq_vector_gen_list
    return np.linalg.norm(diff)


def freq_vector_gen_verified(gen_file,train_dict_keys): #We need the keys of the training data.
    gen_list=load_train_data(gen_file)
    gen_list = [sample for sample in gen_list if verify_sequence(sample)]
...
def freq_compare_verified(gen_file,train_file):
    freq_vector_train_list, train_dict_keys= freq_vector_train(train_file)
    freq_vector_gen_list=freq_vector_gen_verified(gen_file,train_dict_keys)
    diff= freq_vector_train_list- freq_vector_gen_list
    return np.linalg.norm(diff)



def freq_vector_train_list(train_list):
    bigstring= ''.join(train_list)
    occur_dict = collections.Counter(bigstring)
    occur_vect=[occur_dict[x] for x in occur_dict.keys()]
    freq_list=[float(x)/len(bigstring) for x in occur_vect] 
...
def freq_vector_gen_verified_list(gen_list,train_dict_keys): #We need the keys of the training data.
#    gen_list=load_train_data(gen_file)
    gen_list = [sample for sample in gen_list if verify_sequence(sample)]
...
def freq_compare_verified_list(gen_list,train_list):
    freq_train_list, train_dict_keys= freq_vector_train_list(train_list)
    freq_vector_gen_list=freq_vector_gen_verified_list(gen_list,train_dict_keys)
    diff= freq_train_list- freq_vector_gen_list
    return np.linalg.norm(diff)



#train_list=read_smi('drugs_sub.smi')
#indices_100=np.argpartition(mol_drug_qed,-100)[-100:] 
#mol_new_100=mol_100[indices_100]                     
#mol_100=np.array(mol_drug_qed)
#mol_drug_qed=[qed.default(m) for m in mol_drug_chem]
#mol_drug_chem=[Chem.MolFromSmiles(m) for m in mol_drug_ver]
#mol_drug_ver = [sample for sample in organ_drug if verify_sequence(sample)]
#organ_drug=read_smi('drugs_08_drug-candidate_199.smi') 
